...
class CA_Excel_Copilot:
    def __init__(self, root):
        ...
        # 1. Load the shared API key from our Universal Setup
        self.config_file = os.path.join(os.path.expanduser("~"), ".ca_auditor", "config.json")
        self.keys = self.load_keys()
        if not self.keys or "GROQ_API_KEY" not in self.keys:
            self.root.destroy()
            return
            
        self.groq_client = Groq(api_key=self.keys.get("GROQ_API_KEY"))
    ...
    def load_keys(self):
        if os.path.exists(self.config_file):
            with open(self.config_file, 'r') as f:
                return json.load(f)
        messagebox.showerror("Auth Error", "Run the main CA Auditor App first to set up your Groq API key.")
        return None
    ...
    def execute_command(self):
        ...
        try:
            # Using the specific OSS 120b model requested
